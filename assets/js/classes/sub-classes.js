class Person {
    static _count = 0;

    static get count() {
        return `There are ${Person._count} instances of Person class`;
    }

    static message() {
        console.log(this.code);
        console.log('Hello from Person class');
    }

    name = '';
    code = '';
    phrase = '';
    food = '';

    constructor(name = 'No name', code = 'No code', phrase = 'No phrase') {
        this.name = name;
        this.code = code;
        this.phrase = phrase;

        Person._count++;
    }

    /**
     * @param {string} food
     * @returns {void}
     */
    set setFavoriteFood(food) {
        this.food = food.toUpperCase();
    }

    /**
     * @returns {string}
     */
    get getFavoriteFood() {
        return `The favorite food of ${this.name} is ${this.food}`;
    }

    whoAmI() {
        console.log(`I'm ${this.name} and my code is ${this.code}`);
    }

    myPhrase() {
        this.whoAmI();

        console.log(`${this.code} says: ${this.phrase}`);
    }
}

class Hero extends Person {
    clan = '';

    constructor(name, code, phrase, clan = 'No clan') {
        super(name, code, phrase);

        this.clan = clan;
    }

    whoAmI() {
        super.whoAmI();

        console.log(`I'm from ${this.clan}`);
    }
}

// const per1 = new Person('Peter Parker', 'Spiderman', 'I\'m your friendly neighborhood Spiderman');
const her1 = new Hero('Peter Parker', 'Spiderman', 'I\'m your friendly neighborhood Spiderman', 'Avengers');

console.log(her1);

her1.whoAmI();
